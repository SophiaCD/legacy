...
async def on_message(message):
    if message.author == client.user:
        ...
        newBoard = CreateBoard(int(coords[0])+1, int(coords[1])+1)
        response = ""
        for i in range(0,len(newBoard)):
            response = response + newBoard[i]
    ...
    #elif message.content.lower().startswith("cf"):
        #try:
    #    response = TicTac(int(message.content[2:]), message)
